...
- [x] Check that d is the same as the original code
    - The chunked DAXPY result matches the standard DAXPY result. `Total absolute error between standard and chunked DAXPY results: 0`
- [x] Check that this last sum is the same as the sum of all the elements of d in the original code.
  - The last sum of `partial_chunck_sum` is not the same as the sum of all elements of d in the original code. The differences are:
    - Difference between chunked long double and chunked double: 1.1368683772161603e-13
    - Difference with long double: 1.1368683772161603e-13
    - Difference with double: 2.8421709430404007e-13
  - While the sum of chunked DAXPY result vector is the same as the sum of all elements of d in the original code:
    - Final difference from result vector 0 is not less than epsilon 0
  - So, I guess that the difference between the sum of `partial_chunk_sum` and the sum of all elements of d in the original code is due to that in the chunked summation, the intermediate sums will lose some precision. Instead, if long double is used at the beginning, the precision loss can be reduced.
# Source Code
